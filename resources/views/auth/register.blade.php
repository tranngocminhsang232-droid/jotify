@extends('layouts.app')
@section('title', 'Create Account - JOTIFY')

{{-- Same layout as login page but opens on register panel --}}
@section('content')
@php
// Pass register errors to the view
$hasRegisterErrors = $errors->any();
@endphp
...
.auth-track-wrap { overflow: hidden; width: 100%; position: relative; }
.auth-track {
...
.auth-panel {
    ...
    overflow: hidden; padding: 0 1rem; box-sizing: border-box;
}
.auth-panel > * { max-width: 420px; margin-left: auto; margin-right: auto; }
.auth-panel form { max-width: 420px; width: 100%; margin: 0 auto; }
.auth-panel .panel-heading,
.auth-panel .auth-divider,
.auth-panel .auth-footer,
.auth-panel .auth-alert-error { max-width: 420px; margin-left: auto; margin-right: auto; }
...
.auth-input-clean {
    ...
    background: var(--color-body-bg); border: 1.5px solid var(--color-border);
...
.auth-submit {
    ...
    padding: 0.78rem 1.25rem; background: var(--accent-dim); color: #fff;
...
.auth-suggest {
    ...
    border-radius: 0.75rem; overflow: hidden; z-index: 40; box-shadow: 0 6px 20px rgba(0,0,0,0.1);
}
...
<div class="auth-wrap"
    ...
    <div class="auth-brand">
        <div class="brand-logo-ring"><img src="{{ asset('jotify-logo.png') }}" alt="JOTIFY"></div>
        ...
        <div class="auth-track-wrap">
            <div class="auth-track" :class="mode === 'login' ? 'show-login' : 'show-register'">
                ...
                <div class="auth-panel">
                    ...
                    <form action="/login" method="POST" id="login-form">
                        ...
                        <div class="auth-field" x-data="emailSuggest()">
                            ...
                            <div class="auth-field-wrap">
                                ...
                                <div class="auth-suggest" x-show="showSuggestion && suggestion"
                        ...
                        <div class="auth-field">
                            ...
                            <div class="auth-field-wrap">
                                ...
                                <input :type="showPassLogin ? 'text' : 'password'" id="pw_login" name="password" required class="auth-input-clean has-toggle" placeholder="••••••••">
                                <button type="button" @click="showPassLogin = !showPassLogin" class="icon-btn !p-1" style="position:absolute;right:0.7rem;top:50%;transform:translateY(-50%);">
                                    <span class="material-icons-outlined" style="font-size:1.05rem;" x-text="showPassLogin ? 'visibility_off' : 'visibility'"></span>
                                </button>
                            </div>
                        </div>
                        <div class="auth-actions">
                            <label class="auth-remember"><input type="checkbox" name="remember" id="remember"> Remember me</label>
                    ...
                    <p class="auth-footer">Don't have an account? <button type="button" @click="setMode('register')">Create one →</button></p>
                </div>
                ...
                <div class="auth-panel">
                    ...
                    <form action="/register" method="POST">
                        ...
                        <div class="auth-field">
                            ...
                            <div class="auth-field-wrap">
                                <span class="auth-field-icon material-icons-outlined">mail</span>
                                <input type="email" id="reg_email" name="email" value="{{ old('email') }}" required class="auth-input-clean" placeholder="you@example.com">
                            </div>
                        </div>
                        <div class="auth-field">
                            ...
                            <div class="auth-field-wrap">
                                ...
                                <input type="text" id="display_name" name="display_name" value="{{ old('display_name') }}" required class="auth-input-clean" placeholder="John Doe">
                            </div>
                        </div>
                        <div class="auth-field">
                            ...
                            <div class="auth-field-wrap">
                                <span class="auth-field-icon material-icons-outlined">lock</span>
                                <input :type="showPassReg ? 'text' : 'password'" id="pw_reg" name="password" required class="auth-input-clean has-toggle" placeholder="Min. 8 characters">
                                <button type="button" @click="showPassReg = !showPassReg" class="icon-btn !p-1" style="position:absolute;right:0.7rem;top:50%;transform:translateY(-50%);">
                                    <span class="material-icons-outlined" style="font-size:1.05rem;" x-text="showPassReg ? 'visibility_off' : 'visibility'"></span>
                                </button>
                            </div>
                        </div>
                        <div class="auth-field">
                            ...
                            <div class="auth-field-wrap">
                                ...
                                <input :type="showPassReg ? 'text' : 'password'" id="pw_confirm" name="password_confirmation" required class="auth-input-clean has-toggle" placeholder="Repeat password">
                            </div>
                        </div>
                    ...
                    <div class="auth-divider"></div>
                    <p class="auth-footer">Already have an account? <button type="button" @click="setMode('login')">Sign in →</button></p>
                </div>
...
function emailSuggest() {
    return {
        ...
        init(){
            ...
            setTimeout(()=>{const el=this.$refs.emailInput;if(el&&!el.matches(':focus'))el.value='';},80);
        },
        onFocus(){clearTimeout(this._blurTimer);const cur=this.$refs.emailInput.value.trim();if(this.suggestion&&(cur===''||this.suggestion.toLowerCase().startsWith(cur.toLowerCase())))this.showSuggestion=true;},
        ...
        applySuggestion(){this.$refs.emailInput.value=this.suggestion;this.showSuggestion=false;this.$refs.emailInput.focus();}
    };
}
document.addEventListener('DOMContentLoaded',()=>{
    ...
    form.addEventListener('submit',()=>{
        ...
        if(!emailEl)return;
        emailEl.name='email';
        if(remember?.checked&&emailEl.value)localStorage.setItem('rememberedEmail',emailEl.value.trim());
