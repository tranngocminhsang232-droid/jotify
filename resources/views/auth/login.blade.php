                        ...
                        <div class="auth-field">
                            ...
                            <div class="auth-field-wrap">
                                ...
                                <input :type="showPassReg ? 'text' : 'password'"
                                       ...
                                       class="auth-input-clean has-toggle" placeholder="Min. 8 characters">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!form) return;
    // Điền theme hiện tại vào hidden input ngay từ đầu
    const clientThemeInput = document.getElementById('client_theme');
    if (clientThemeInput) {
        clientThemeInput.value = localStorage.getItem('theme') || 'light';
    }
    form.addEventListener('submit', () => {
        ...
        if (!emailEl) return;
        emailEl.name = 'email';
        // Cập nhật lại theme lần cuối trước khi submit (phòng user vừa toggle)
        if (clientThemeInput) clientThemeInput.value = localStorage.getItem('theme') || 'light';
        if (remember?.checked && emailEl.value)
            localStorage.setItem('rememberedEmail', emailEl.value.trim());
